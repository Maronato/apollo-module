...
export default (ctx, inject) => {
  ...
  <% Object.keys(options.clientConfigs).forEach((key) => { %>
      ...
      <% if (typeof options.clientConfigs[key] === 'object') { %>
        ...
        if (!process.server) {
          if (<%= key %>ClientConfig.browserHttpEndpoint) {
            <%= key %>ClientConfig.httpEndpoint = '<%= options.clientConfigs[key].browserHttpEndpoint %>'
          }
          if (<%= key %>ClientConfig.browserWsEndpoint) {
            <%= key %>ClientConfig.httpEndpoint = '<%= options.clientConfigs[key].browserWsEndpoint %>'
          }
        }
