...
export default (ctx, inject) => {
  ...
  <% Object.keys(options.clientConfigs).forEach((key) => { %>
      ...
      if (!process.server) {
        if (options.clientConfigs[key].browserHttpEndpoint) {
          options.clientConfigs[key].httpEndpoint = '<%= options.clientConfigs[key].browserHttpEndpoint %>'
        }
        if (options.clientConfigs[key].browserWsEndpoint) {
          options.clientConfigs[key].httpEndpoint = '<%= options.clientConfigs[key].browserWsEndpoint %>'
        }
      }
